...
class BLTI {
    ...
    function __construct($consumer=false, $shared_secret=false, $usesession=true, $doredirect=true) {
       $this->server_input = filter_input_array(INPUT_SERVER, FILTER_SANITIZE_FULL_SPECIAL_CHARS); //Sanitize $_SERVER global
       $this->post = filter_input_array(INPUT_POST, FILTER_SANITIZE_FULL_SPECIAL_CHARS); //Sanitize $_POST global
